...
/**
  ...
  * 要求特征值非负
  *
  * 先使用 NaiveBayesDataProduce 产生数据
  */
object NaiveBayesDemo {
  ...
  def main(args: Array[String]): Unit = {
    val conf: SparkConf = new SparkConf().setMaster("local[*]").setAppName("NaiveBayesDemo")
    val sc = new SparkContext(conf)

    val data: RDD[LabeledPoint] = MLUtils.loadLibSVMFile(sc, "data/output")
