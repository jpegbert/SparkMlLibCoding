...
object Isotonic_Regression {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val source: BufferedSource = Source.fromFile("data/sample_isotonic_regression_data.txt")
    val lines: Iterator[String] = source.getLines()
    val array: Array[String] = lines.toArray

    val conf: SparkConf = new SparkConf().setMaster("local[*]").setAppName("Isotonic_Regression")
